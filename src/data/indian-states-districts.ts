...
export const INDIAN_STATES_AND_DISTRICTS: StateDistrictData[] = [
  {
  ...
  },
  {
    state: 'Chhattisgarh',
    code: 'CT',
    districts: ['Balod', 'Baloda Bazar', 'Balrampur', 'Bastar', 'Bemetara', 'Bijapur', 'Bilaspur', 'Dantewada', 'Dhamtari', 'Durg', 'Gariaband', 'Gaurela-Pendra-Marwahi', 'Janjgir-Champa', 'Jashpur', 'Kabirdham', 'Kanker', 'Kondagaon', 'Korba', 'Koriya', 'Mahasamund', 'Manendragarh-Chirmiri-Bharatpur', 'Mohla-Manpur', 'Mungeli', 'Narayanpur', 'Raigarh', 'Raipur', 'Rajnandgaon', 'Sarangarh-Bilaigarh', 'Sukma', 'Surajpur', 'Surguja']
  },
  {
    state: 'Goa',
    code: 'GA',
...
export const getDistrictsByState = (stateName: string): string[] => {
  const stateData = INDIAN_STATES_AND_DISTRICTS.find(state => 
...
export const getStateByDistrict = (districtName: string): string | null => {
  for (const stateData of INDIAN_STATES_AND_DISTRICTS) {
    if (stateData.districts.some(district => 
      district.toLowerCase() === districtName.toLowerCase()
    )) {
...
export const searchDistricts = (query: string): { district: string; state: string }[] => {
  ...
  INDIAN_STATES_AND_DISTRICTS.forEach(stateData => {
    stateData.districts.forEach(district => {
      if (district.toLowerCase().includes(lowerQuery)) {
        results.push({ district, state: stateData.state });
      }
    });
  });
